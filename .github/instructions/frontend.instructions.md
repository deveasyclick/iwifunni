...
- **Hook return types go in `types/`, not in hook files.** Never define or export types from a hook file. Place all hook return type interfaces in the relevant `types/` file (e.g., `UsePayloadEditorReturn` goes in `types/data-panel.ts`) and import them back into the hook.
