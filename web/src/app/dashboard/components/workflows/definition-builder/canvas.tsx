...
export const WorkflowBuilderCanvas = ({
  ...
  const innerCanvasHeight = useMemo(() => {
    ...
    return Math.max(5200, lowestNodeBottom + 2200);
  }, [nodes]);
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-3xl border border-border/20 bg-dark p-3 text-white shadow-lg">
        <div className="mb-3 flex items-center justify-between gap-3 px-2 py-1">
          <div>
            <h6 className="font-medium text-white">Workflow Canvas</h6>
            <p className="text-sm text-bodytext">
              Start from the trigger node, then insert actions from the edge controls to keep the flow linear.

              Click any node to inspect it. Use the edge plus buttons to insert the next step.
            </p>
          </div>
          <span className="text-xs text-bodytext">
            {nodes.length} node{nodes.length === 1 ? "" : "s"} · {edgeCount} edge{edgeCount === 1 ? "" : "s"}
          </span>
        </div>

        <div className="scrollbar-none relative h-168 overflow-x-hidden overflow-y-auto overscroll-contain rounded-3xl border border-border/20 bg-dark [-ms-overflow-style:none] before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:h-32 before:bg-linear-to-b before:from-lightprimary before:to-transparent before:opacity-70 before:content-[''] [&::-webkit-scrollbar]:h-0 [&::-webkit-scrollbar]:w-0 [&_.react-flow__attribution]:hidden [&_.react-flow__controls]:overflow-hidden [&_.react-flow__controls]:rounded-2xl [&_.react-flow__controls]:border [&_.react-flow__controls]:border-border/20 [&_.react-flow__controls]:bg-dark [&_.react-flow__controls]:shadow-lg [&_.react-flow__controls-button]:h-9! [&_.react-flow__controls-button]:w-9! [&_.react-flow__controls-button]:border-border/20! [&_.react-flow__controls-button]:bg-transparent! [&_.react-flow__controls-button]:text-bodytext! [&_.react-flow__controls-button:hover]:bg-lightprimary/15! [&_.react-flow__controls-button:hover]:text-primary!">
          <div className={`relative min-h-full min-w-full ${innerCanvasHeightClass}`}>
            <ReactFlow<WorkflowCanvasNode, WorkflowCanvasEdge>
              ...
              connectionLineStyle={{ stroke: "var(--primary)", strokeWidth: 2.2 }}
              deleteKeyCode={null}
